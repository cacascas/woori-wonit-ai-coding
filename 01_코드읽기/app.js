...
q("#acc").innerHTML =
  ...
  d
    .map(
      (x) => `
        <div class="row">
          <b>${x.ownerName}</b> · ${x.accountType} · ${f2(x.accountNo)}<br>
          ${f1(x.balance)}
          <span class="${x.status === "정상" ? "ok" : "bad"}">${x.status}</span>
        </div>
      `,
    )
    .join("");
...
q("#tx").innerHTML =
  ...
  tmp
    .map(
      (x) => `
        <div class="row">
          ${x.txDatetime.slice(0, 10)} · ${x.memo}<br>
          <span class="${x.txType === "입금" ? "ok" : "bad"}">
            ${x.txType === "입금" ? "+" : "-"}${f1(x.amount)}
          </span>
        </div>
      `,
    )
